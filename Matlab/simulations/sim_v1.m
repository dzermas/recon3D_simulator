...
P = zeros(3, N_points); %% Euclidean (euclidean values)
...
% Translation
C2_P_C1 = [0.5 * rand; 0.5 * rand ; 0.5 * rand];
...
for i = 1:N_points
    ...
    C2_euclidean = C2_R_C1 * (euclidean + g_noise_2) + C2_P_C1;
...
epipolar_b = diag( C2_h' * E_bearings * C1_h );
...
epipolar = diag( C2_h' * E_estimated * C1_h );
...
%% Kanatani correction
reps = 3;
[v_k, u_k, Dv_k, Du_k, keepers, R2_k, h2_k] = ...
...
new_Essential = skew_sym(new_t2) * new_R2;
